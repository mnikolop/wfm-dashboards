...
def customerContracts_def(size = 50, start = 1111):

    # Creates customer contract data.
    # Prerequisites: cost centers data to be created in the warehouse.
...
def customers_def(size = 50, start = 1111):

    # Creates customer data.
    # Prerequisites: d_customerContracts. Companies list file to be uploaded to the warehouse
    # Inputs: size: the number of cost centers to be breated.
            # start: the starting point for the index. default is 1111. NOTE: if there is data in the warehouse use the last index+1 as the start and switch the write line to the append option
    ...
    delta =spark.read.format('delta').table("d_customerContracts")
    customerContracts = delta.toPandas()
